...
class EntityExtractor:
        ...
        try:
            ...
            for key, value in current_entities.to_dict().items():
                if key in extracted_dict:
                    if extracted_dict[key] or not value:  # Если в API есть значение или текущее пустое
                        setattr(new_entities, key, extracted_dict[key])
                    else:  # Иначе сохраняем текущее
            ...
            # Используем резервные алгоритмы
    ...
    def _extract_entities_fallback(self, message: str, current_entities: TravelEntities) -> TravelEntities:
        ...
        Returns:
            Обновленные сущности
        """
        # Копируем текущие сущности для обновления
        ...
        if cities["mid_city"]:
            # Объединяем списки городов без дубликатов
            mid_cities = set(extracted["mid_city"])
            for city in cities["mid_city"]:
                mid_cities.add(city)
