...
class MistralAIClient:
    ...
    def _build_extraction_prompt(self, message: str, context: Dict[str, Any]) -> str:
        ...
        - start_city: Город отправления на русском языке в именительном падеже
        - end_city: Город прибытия на русском языке в именительном падеже
        ...
        3. Возвращай названия только на русском языке в именительном падеже
        4. Если сущность уже есть в текущих данных и не упоминается в новом сообщении, сохрани существующее значение
        5. Верни ТОЛЬКО JSON-объект без пояснений. Формат:
        {{
          "date": "дд.мм.гггг",
